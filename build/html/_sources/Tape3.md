...
- `start` est l'équivalent de `run` avec un breakpoint sur `main()`. Vu que le binaire ne contient pas assez de symboles pour le débugage, on ne peut faire des breakpoint que sur des adresses.
...
- On peut avoir l'assembleur d'une fonction en utilisant la commande `disassemble`, exemple: `disassemble start_level` donnera le même code assembleur de la fonction `start_level`. C'est plus pratique sur la VM vu que `objdump --disassemble=start_level` n'est pas supporté par cette dernière.
...
> Si vous ca ne marche pas avec `start` alors que le shellcode est bel et bien exécuté, essayez avec `run` et en utilisant un fichier au lieu d'une commande bash:
> ```sh
